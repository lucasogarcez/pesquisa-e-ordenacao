...
void pre_ordem(Pagina *raiz);
void mostrar_arvoreB(Arvoreb *tree);
void mostrar_lista(Listad *lista);
...
// qtde é a quantidade de elementos
// da primeira lista depois da divisão
    ...
    if (aux_pagina != NULL)
    {
        while (aux_pagina->folha == 0) // enqto nao for folha
        {
            ...
            while (aux_lista != NULL &&
                   get_valor_chave(aux_lista) < novo_valor_chave)
            {
...
Pagina *divide_pagina(Pagina *pagina_a_dividir)
{
    ...
    ultima_chave = pagina_a_dividir->listaChaves->fim;
    ...
    if (pagina_a_dividir->pai != NULL)
    {
        ...
        while (aux_lista != NULL && 
            get_filho(aux_lista) != pagina_a_dividir)
            {
                aux_lista = aux_lista->prox;
            }
        if (aux_lista != NULL)
    ...
    if(!pagina_a_dividir->folha)
    {
        ...
        pagina_a_dividir->direita = get_filho(ultima_chave);
...
void insere_ordenado(Listad *lista, Chave *ch)
{
    ...
    {  
        if(ch->valorChave < get_valor_chave(lista->ini))
           ...
           if(ch->valorChave > get_valor_chave(lista->fim))
...
void pre_ordem(Pagina *raiz) {
    if (raiz != NULL) {
        mostrar_lista(raiz->listaChaves);
...
int main()
{
    int n, m;
    scanf("%d", &n);
    Arvoreb *tree = cria_arvoreb(n);

    while (scanf("%d", &m), m != -1) {
        insere_chave_arvoreb(tree, m);
    }

    mostrar_arvoreB(tree);
